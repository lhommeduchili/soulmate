...
const PLATFORMS = [
  {
    name: "darwin-x64",
    asset: `slskd-${VERSION}-osx-x64.zip`,
    binary: "slskd",
  },
  {
    name: "darwin-arm64",
    asset: `slskd-${VERSION}-osx-arm64.zip`,
    binary: "slskd",
  },
  {
    name: "win32-x64",
    asset: `slskd-${VERSION}-win-x64.zip`,
    binary: "slskd.exe",
  },
  {
    name: "linux-x64",
    asset: `slskd-${VERSION}-linux-x64.zip`,
    binary: "slskd",
  },
];
    ...
    try {
      ...
      if (!plat.name.startsWith("win")) {
        ...
        if (plat.name.startsWith("darwin")) {
          console.log(
            `[Binaries] Ad-hoc signing ${plat.name}...`,
          );
          try {
            execSync(
              `codesign --force --sign "-" "${finalBinaryPath}"`,
            );
