...
export class SpotifyService {
  ...
  private getMarket(): string {
    return this.preferencesService.getRegion();
  }
  ...
  async login(): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      ...
      const handleUrl = (url: string) => {
        if (url.startsWith("soulmate://")) {
          ...
          this.handleCallback(url).catch((err) => {
            console.error(
              "[SpotifyService] Error treating url in window:",
              err,
            );
          });
        }
      ...
      this.authWindow.webContents.on("will-redirect", (_, url) =>
        handleUrl(url),
      );
      this.authWindow.webContents.on("will-navigate", (_, url) =>
        handleUrl(url),
      );
  ...
  async handleCallback(url: string): Promise<boolean> {
    if (this.isProcessingCallback) {
      console.log(
        "[SpotifyService] Callback already being processed, ignoring duplicate",
      );
      return true;
    }
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          "[SpotifyService] Token exchange failed:",
          await response.text(),
        );
        return false;
      }
  ...
  async getPlaylists(): Promise<Playlist[]> {
    ...
    const response = await net.fetch(
      `https://api.spotify.com/v1/me/playlists?market=${encodeURIComponent(market)}`,
      {
  ...
  async getPlaylistTracks(playlistId: string): Promise<Track[]> {
    ...
    if (!token) {
      console.log(
        "[SpotifyService] No token, attempting to scrape tracks for:",
        playlistId,
      );
    ...
    const market = this.getMarket();
    let url = `https://api.spotify.com/v1/playlists/${playlistId}/tracks?limit=100&market=${encodeURIComponent(market)}`;
    let allTracks: any[] = [];
  ...
  async searchPlaylists(query: string): Promise<Playlist[]> {
    ...
    const market = this.getMarket();
    const response = await net.fetch(
      `https://api.spotify.com/v1/search?type=playlist&q=${encodeURIComponent(query)}&limit=20&market=${encodeURIComponent(market)}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    );
    ...
    const data = (await response.json()) as {
      playlists: SpotifyPaging<Playlist>;
    };
  ...
  async getPlaylist(idOrUrl: string): Promise<Playlist | any> {
    ...
    if (!token) {
      console.log(
        "[SpotifyService] No token, attempting to scrape metadata for:",
        playlistId,
      );
  ...
  private async scrapePlaylistTracks(playlistId: string): Promise<Track[]> {
    ...
    try {
      ...
      for (let i = 0; i < trackMatches.length; i++) {
        ...
        const endIdx =
          i < trackMatches.length - 1
            ? trackMatches[i + 1].index!
            : html.length;
      ...
      log.info(
        `[SpotifyService] Scraped ${uniqueTracks.length} tracks (via net.fetch)`,
      );
      return uniqueTracks;
    } catch (e) {
