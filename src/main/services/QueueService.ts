...
export class QueueService {
  ...
  constructor(
    ...
    const storedPriority = this.preferencesService.getFormatPriority();
    this.matchMaker = new MatchMaker(storedPriority);
  ...
  setFormatPriority(formats: string[]) {
    ...
    this.preferencesService.setFormatPriority(formats);
    log.info("[QueueService] Format priority updated:", formats);
  }

  getFormatPriority(): string[] {
    return this.preferencesService.getFormatPriority();
  }
    ...
    if (item) {
      if (item.status === "downloading" || item.status === "queued") {
        if (item.username && item.transferId) {
          try {
            await this.slskdClient.cancelDownload(
              item.username,
              item.transferId,
            );
            log.info(
              `[QueueService] Remote download cancelled: ${item.transferId}`,
            );
          } catch (e) {
            log.warn(
              `[QueueService] Failed to cancel remote download (might already be gone): ${e}`,
            );
          }
        }
      }
  ...
  private async processQueue() {
    ...
    log.info(
      `[QueueService] Starting processing for ${pendingItems.length} items...`,
    );
  ...
  private async searchAndDownload(item: DownloadItem) {
    ...
    this.emitQueueUpdate();
    log.info(
      `[QueueService] Searching for: ${item.track.name} - ${item.track.artists[0].name}`,
    );
    ...
    try {
      ...
      while (attempts < maxAttempts) {
        ...
        const searchState = await this.slskdClient.getSearchResults(
          searchInit.id,
        );
        ...
        if (searchState.responses && searchState.responses.length > 0) {
          ...
          console.log(
            `[QueueService] Flattened ${allFiles.length} files from ${searchState.responses.length} responses`,
          );
          if (allFiles.length > 0) {
            console.log(
              `[QueueService] Sample file structure:`,
              JSON.stringify(allFiles[0], null, 2),
            );
          }
          ...
          if (bestMatch) {
            console.log(
              `[QueueService] Found match: ${bestMatch.filename} from ${bestMatch.username}`,
            );
            break;
          }
        }
      ...
      if (bestMatch) {
        ...
        try {
          ...
          const response = await this.slskdClient.download(
            bestMatch.username,
            bestMatch.filename,
            bestMatch.size,
          );
          ...
          log.info(
            `[QueueService] Download started for ${item.id} (Transfer ID: ${item.transferId})`,
          );
        } catch (e) {
    ...
    try {
      ...
      for (const item of activeDownloads) {
        ...
        for (const userTransfer of transfers) {
          if (
            userTransfer.username !== item.username &&
            userTransfer.Username !== item.username
          )
            continue;

          const dirs =
            userTransfer.directories || userTransfer.Directories || [];
          for (const dir of dirs) {
            ...
            for (const file of files) {
              if (
                ...
                file.Filename === item.filename ||
                (item.transferId &&
                  (file.id === item.transferId || file.Id === item.transferId))
              ) {
                remoteFile = file;
                break;
              }
        ...
        if (remoteFile) {
          ...
          item.progress = Math.min(
            100,
            Math.floor((bytesTransferred / size) * 100),
          );
